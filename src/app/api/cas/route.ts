...
import { generateCodeCas, generatePatientId } from "@/utils/generate-id"
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Resolve service code for codeCas generation
    const etablissementId = body.etablissementId || session.user.etablissementId || null
    let serviceCode: string | null = null
    if (etablissementId) {
      const etab = await prisma.etablissement.findUnique({
        where: { id: etablissementId },
        select: { codeService: true },
      })
      serviceCode = etab?.codeService ?? null
    }
    const declarationYear = body.anneeDeclaration ?? new Date().getFullYear()
    let codeCim10: string | null = null
    if (body.maladieId) {
      const mal = await prisma.maladie.findUnique({
        where: { id: body.maladieId },
        select: { codeCim10: true },
      })
      codeCim10 = mal?.codeCim10 ?? null
    }
    const codeCas = await generateCodeCas(declarationYear, serviceCode, codeCim10, prisma)
