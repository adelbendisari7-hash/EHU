...
const BREADCRUMBS: Record<string, { label: string; parent?: string }> = {
  ...
  "/declarations": { label: "Cas Déclarés" },
...
function Breadcrumb({ pathname }: { pathname: string }) {
  ...
  // Handle dynamic routes like /declarations/[id]
  if (!current && pathname.startsWith("/declarations/") && pathname !== "/declarations/new") {
    crumbs.push({ label: "Cas Déclarés", href: "/declarations" })
    crumbs.push({ label: "Détail du cas" })
  }
    ...
    <div className="flex items-center gap-1.5">
      {crumbs.map((crumb, i) => (
        <span key={i} className="flex items-center gap-1.5">
          ...
          {crumb.href ? (
            <Link href={crumb.href} className="text-[13px] text-gray-400 hover:text-gray-600 transition-colors">
              {crumb.label}
            </Link>
          ) : (
...
export default function Topbar({ userName }: TopbarProps) {
  ...
  return (
    <header className="fixed top-0 right-0 h-14 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-4 lg:px-6 z-20 left-0 lg:left-64">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <span className="text-[13px] text-gray-700 font-medium hidden sm:block">{userName}</span>
        </Link>
      </div>
    </header>
  )
